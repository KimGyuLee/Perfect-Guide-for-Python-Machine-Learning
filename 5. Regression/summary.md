...
* **RSS (Residual Sum of Squares)** : 오류 값의 제곱을 구해서 더하는 방식이다.  
  ...
  - RSS는 회귀식의 독립변수 X, 종속변수 Y가 중심 변수가 아니라 W 변수 (회귀 계수)가 중심 변수임을 인지하는 것이 매우 중요하다. (학습 데이터로 입력되는 독립 변수와 종속 변수는 RSS에서 모두 상수로 간주한다.)
...
* **선형 회귀 모델을 위한 데이터 변환**
  ...
  - **타겟값은 반드시 정규 분포를 가져야 한다.** 이를 위해 주로 로그 변환을 적용한다.
  - 피처값은 주로 평균이 0, 분산이 1인 표준 정교 분포를 가진 데이터 세트로 변환하거나(StandardScaler), 최소값이 0이고 최댓값이 1인 값으로 정규화를 수행한다(MinMaxScaler).
  - 스케일링/정규화를 수행한 데이터 세트에 다시 다항 특성을 적용하여 변환하는 방법도 고려한다. 보통 StandardScaler, MinMaxScaler 후 예측 성능에 향상이 없을 경우 이 방법을 적용한다.
  - 일반적으로 위의 두가지 방법보다 많이 쓰이는 방법은 타겟값뿐 아니라 피처값도 로그 변환을 하는 방법이다. 첫번째 방법의 경우 예측 성능 향상을 크게 기대하기 어려운 경우가 많고, 두번째 방법의 경우에는 피처의 개수가 매우 많을 경우 다항 변환으로 생성되는 피처의 개수가 기하급수로 늘어나서 과적합 이슈가 발생할 수 있기 때문이다.
  - 선형 회귀의 데이터 인코딩은 일반적으로 레이블 인코딩이 아니라 원-핫 인코딩을 적용한다. 레이블 인코딩은 0, 1, 2, 3... 으로 카테고리값을 인코딩한다. 그러나 0, 1, 2, 3의 숫자의 대소가 영향을 줄 수 있으므로 0 0 0 1, 0 0 1 0 과 같이 원-핫 인코딩을 적용한다.
...
* **규제 방식 L1 & L2**
  ...
  - L1, L2 규제를 결합한 모델을 ElasticNet이라고 한다. 주로 피처가 많은 데이터 세트에 적용하며, L1 규제로 피처의 개수를 줄임과 동시에 L2 규제로 계수 값의 크기를 조정한다.
...
* **개념**
  - 로지스틱 회귀는 선형 회귀 방식을 분류에 적용한 알고리즘이다. 로지스틱 회귀가 선형 회귀와 다른 점은 학습을 통해 선형 함수의 회귀 최적선을 찾는 것이 아니라, 시그모이드 함수의 최적선을 찾고 이 시그모이드 함수의 반환 값을 확률로 간주해 확률에 따라 분류를 결정한다는 것이다. (반환값이 0.5보다 크면 1, 0.5보다 작으면 0)
  - 로지스틱 회귀는 주로 이진 분류(0과 1)에 사용된다. 로지스틱 회귀에서 예측 값은 예측 확률을 의미하며, 예측 값 즉 예측 확률이 0.5 이상이면 1로, 0.5 이하이면 0으로 예측한다. 로지스틱 회귀의 예측 확률은 시그모이드 함수의 출력값으로 계산된다.

* **시그모이드 함수**
  - 단순 선형 회귀 : y=w1x+w0이 있다고 할 때, 로지스틱 회귀는 0과 1을 예측하기에 단순 회귀식은 의미가 없다. 하지만 Odds(성공확률/실패확률)을 통해 선형 회귀식에 확률을 적용할 수 있다.
  - Odds(p) = p/(1-p)
  - 하지만 확률p의 범위가 (0,1)이므로 선형 회귀의 반환값인 (-무한대, +무한대)에 대응하기 위해 로그 변환을 수행하고 이 값에 대한 선형 회귀를 적용한다.
  - Log(Odds(p)) = w1x+w0
  - 해당 식을 데이터 값x의 확률 p로 정리하면 아래와 같다.
  - 로지스틱 회귀는 학습을 통해 시그모이드 함수의 w를 최적화하여 예측하는 것이다.

![시그모이드함수](https://user-images.githubusercontent.com/58073455/73997771-d11ba180-49a2-11ea-8411-7160fba418bc.PNG)  

- 로지스틱 회귀는 희소한 데이터 세트 분류에도 뛰어난 성능을 보여서 텍스트 분류에서도 자주 사용된다.
- 사이킷런은 LogisticRegression 클래스로 로지스틱 회귀를 구현한다. 주요 하이퍼 파라미터로 penalty와 C가 있다. Penalty는 규제(Regularization)의 유형을 설정하며 'l2'로 설정 시 L2 규제를, 'l1'으로 설정 시 L1 규제를 뜻한다. (default는 'l2') C는 규제 강도를 조절하는 alpha 값의 역수이다. C 값이 작을수록 규제 강도가 크다.
...
* **개요**
  ...
  - 이는 트리가 CART(Classification and Regression Tree)를 기반으로 만들어졌기 때문이다. CART는 분류뿐만 아니라 회귀도 가능한 트리 분할 알고리즘이다.
  - CART 회귀 트리는 분류와 유사하게 분할을 하며, 분할 기준은 RSS(SSE)가 최소가 될 수 있는 기준을 찾아서 분할 된다.
  - 최종 분할이 완료된 후 각 분할 영역에 있는 데이터 결정값들의 평균 값으로 학습/예측한다.
